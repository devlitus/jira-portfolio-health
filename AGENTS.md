...
# UI approach: Custom UI (project decision)

This project deliberately uses **Custom UI**, not UI Kit, per explicit user decision (2026-08). This overrides the "only ui-kit" restriction elsewhere in this file for this repository. The frontend is a React + TypeScript app in `src/frontend/`, bundled with esbuild (`npm run build`) into `static/main/bundle.js`, and served via the `static/main` resource. Use `@forge/bridge` (`requestJira`, `invoke`, etc.) for platform access from the browser. Run `npm run build` before `forge deploy`, since the bundle is gitignored.
