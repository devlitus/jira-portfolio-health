...
// computation here, just rendering: header + health, DIMENSIONS y "Why?",
// following the spec's ASCII mockup (§16, §17). "Recommended actions" ya no
// vive aquí (Tarea E.5, plan Adaptación 6) — pasó a la pantalla global
// "Recommended Actions" (Fase D); esta pantalla solo enlaza a ella con un CTA.
...
// divisores suaves `divide-outline-variant/50`).
...
import { CalendarIcon, InfoIcon, LayersIcon, LinkIcon, PackageIcon, UsersIcon, WarningIcon, type IconProps } from './ui/icons';
...
interface ProjectDetailProps {
  ...
  onBack: () => void;
  /** Tarea E.5: CTA al final de la pantalla, navega a la pantalla global "Recommended Actions". */
  onViewRecommendedActions: () => void;
}
...
export const ProjectDetail: React.FC<ProjectDetailProps> = ({
  ...
  onSelectProject,
  onBack,
  onViewRecommendedActions,
}) => {
  const { projectKey, projectName, healthScore, status, trend, dimensions, factors, reason, reasonKind } = detail;
    ...
    <section className="flex flex-col gap-gutter">
      ...
      {reason ? (
        ...
        <>
          ...
          <button
            type="button"
            onClick={onViewRecommendedActions}
            className="self-start font-label-bold text-label-bold text-primary transition-colors hover:text-primary-container"
          >
